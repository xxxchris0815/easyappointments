...
class Google extends EA_Controller
{
     ...
     *
     * Admin-only cleanup for blockers created by the old bidirectional sync.
     */
    public function cleanup_unavailable_events(): void
    {
        try {
            method('post');

            if (session('role_slug') !== DB_SLUG_ADMIN || cannot('edit', PRIV_USERS)) {
                throw new RuntimeException('You do not have the required permissions for this task.');
            }
            ...
            if (!$google_sync) {
                throw new RuntimeException('Google Calendar sync is not enabled for this provider.');
            }
            ...
            $result = $this->google_sync->remove_unavailable_events($provider);
